...
app.get('/api/clients', async (req, res) => {
  const { data, error } = await supabase
    .from('clients')
    .select('id, name, vertical, wp_url, created_at')
    .order('name');
...
app.post('/api/clients', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('clients')
    .insert({ name, vertical, wp_url, wp_username, wp_app_password })
    .select()
    .single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
...
app.put('/api/clients/:id', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('clients')
    .update({ name, vertical, wp_url, wp_username, wp_app_password })
    .eq('id', req.params.id)
    .select()
    .single();
  if (error) return res.status(500).json({ error: error.message });
...
app.post('/api/clients/:id/profile', async (req, res) => {
  const { brand_voice, target_keywords, sitemap_pages } = req.body;
  // Upsert
  const { data, error } = await supabase
    .from('client_profiles')
    .upsert({
...
app.post('/api/clients/:id/sitemap', upload.single('sitemap'), async (req, res) => {
  ...
  try {
    // Try XML sitemap parse
    const result = await xml2js.parseStringPromise(content);
    const urls = result?.urlset?.url || [];
    pages = urls.map(u => ({
      url: u.loc?.[0] || '',
      title: u['news:news']?.[0]?.['news:title']?.[0]
        || u['image:image']?.[0]?.['image:caption']?.[0]
        || extractTitleFromUrl(u.loc?.[0] || '')
    })).filter(p => p.url);
  } catch {
    // Fallback: treat as plain text list (one URL per line)
    pages = content.split('\n')
      .map(line => line.trim())
      .filter(line => line.startsWith('http'))
      .map(url => ({ url, title: extractTitleFromUrl(url) }));
  }

  // Save to profile
  const { error } = await supabase
    .from('client_profiles')
    .upsert({ client_id: req.params.id, sitemap_pages: pages }, { onConflict: 'client_id' });

  if (error) return res.status(500).json({ error: error.message });
  res.json({ success: true, count: pages.length, pages: pages.slice(0, 10) }); // preview first 10
});
...
app.post('/api/clients/:id/rules', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('brand_rules')
    .insert({ client_id: req.params.id, rule })
    .select()
    .single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
...
app.post('/api/clients/:id/generate-titles', async (req, res) => {
  ...
  const { data: profile } = await supabase
    .from('client_profiles')
    .select('brand_voice, target_keywords, sitemap_pages')
    .eq('client_id', req.params.id)
    .single();
  ...
  try {
    ...
    try {
      ...
      parsed = match ? JSON.parse(match[0]) : { titles: [] };
    }

    // Save proposed titles to Supabase
    const toInsert = (parsed.titles || []).map(t => ({
      client_id: req.params.id,
      title: t.title,
    ...
    if (toInsert.length > 0) {
      await supabase.from('blog_titles').insert(toInsert);
    }
...
app.put('/api/titles/:titleId', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('blog_titles')
    .update({ title, target_keyword, status, notes })
...
app.post('/api/generate-blog', async (req, res) => {
  ...
  // Load profile and brand rules
  const [profileResult, rulesResult] = await Promise.all([
    supabase.from('client_profiles').select('brand_voice, sitemap_pages').eq('client_id', clientId).single(),
  ...
  try {
    ...
    // Parse SEO metadata block
    const metaMatch = rawContent.match(/---SEO METADATA---([\s\S]*?)---END METADATA---/);
    let metadata = {};
    if (metaMatch) {
      const metaText = metaMatch[1];
      metadata.titleTag = (metaText.match(/Title Tag:\s*(.+)/) || [])[1]?.trim();
      metadata.metaDescription = (metaText.match(/Meta Description:\s*(.+)/) || [])[1]?.trim();
      metadata.slug = (metaText.match(/Slug:\s*(.+)/) || [])[1]?.trim();
      metadata.targetKeyword = (metaText.match(/Target Keyword:\s*(.+)/) || [])[1]?.trim();
      metadata.internalLinksUsed = (metaText.match(/Internal Links Used:\s*([\s\S]+)/) || [])[1]?.trim();
    }

    // Strip metadata block from content for clean storage
    const cleanContent = rawContent.replace(/---SEO METADATA---[\s\S]*?---END METADATA---/, '').trim();
    ...
    if (saveError) {
      ...
      return res.json({ content: cleanContent, metadata, saved: false });
    }

    // Mark title as 'in_progress' if this came from a title
    if (titleId) {
...
app.get('/api/clients/:id/blogs', async (req, res) => {
  const { data, error } = await supabase
    .from('blogs')
    .select('id, title, target_keyword, status, is_one_off, created_at, slug, wp_post_id')
    .eq('client_id', req.params.id)
    .order('created_at', { ascending: false });
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
...
app.get('/api/blogs/:blogId/export', async (req, res) => {
  ...
  try {
    ...
    const filename = `${blog.slug || slugify(blog.title)}.docx`;
...
async function generateDocx(blog) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('# ')) {
      children.push(new Paragraph({
        ...
        children: [new TextRun({ text: trimmed.slice(2), bold: true, size: 36, font: 'Poppins' })]
      }));
    } else if (trimmed.startsWith('## ')) {
      children.push(new Paragraph({
        heading: HeadingLevel.HEADING_2,
        children: [new TextRun({ text: trimmed.slice(3), bold: true, size: 28, font: 'Poppins' })]
      }));
    } else if (trimmed.startsWith('### ')) {
      children.push(new Paragraph({
        heading: HeadingLevel.HEADING_3,
        children: [new TextRun({ text: trimmed.slice(4), bold: true, size: 24, font: 'Poppins' })]
      }));
    } else if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
      children.push(new Paragraph({
        numbering: { reference: 'bullets', level: 0 },
        children: [new TextRun({ text: trimmed.slice(2), size: 22, font: 'Lato' })]
      }));
    } else if (trimmed.startsWith('[Image:')) {
      children.push(new Paragraph({
        children: [new TextRun({ text: trimmed, italics: true, color: '888888', size: 20, font: 'Lato' })]
      }));
    } else {
  ...
  // Append SEO metadata section
  if (blog.title_tag || blog.meta_description || blog.slug) {
    children.push(new Paragraph({ children: [new TextRun('')] }));
    children.push(new Paragraph({
      children: [new TextRun({ text: 'SEO Metadata', bold: true, size: 24, font: 'Poppins', color: '28AB83' })]
    }));

    const metaFields = [
      ['Title Tag', blog.title_tag],
      ['Meta Description', blog.meta_description],
      ['Slug', blog.slug],
      ['Target Keyword', blog.target_keyword],
      ['Internal Links', blog.internal_links_used]
    ];
    ...
    for (const [label, value] of metaFields) {
      if (value) {
        children.push(new Paragraph({
          children: [
            new TextRun({ text: `${label}: `, bold: true, size: 20, font: 'Lato' }),
            new TextRun({ text: value, size: 20, font: 'Lato' })
          ]
        }));
      }
    }
      ...
      paragraphStyles: [
        {
          ...
          run: { size: 36, bold: true, font: 'Poppins', color: '323547' },
          paragraph: { spacing: { before: 360, after: 240 }, outlineLevel: 0 }
        },
        {
          id: 'Heading2', name: 'Heading 2', basedOn: 'Normal', next: 'Normal', quickFormat: true,
          run: { size: 28, bold: true, font: 'Poppins', color: '323547' },
          paragraph: { spacing: { before: 280, after: 160 }, outlineLevel: 1 }
        },
        {
          id: 'Heading3', name: 'Heading 3', basedOn: 'Normal', next: 'Normal', quickFormat: true,
          run: { size: 24, bold: true, font: 'Poppins', color: '323547' },
...
function parseInlineFormatting(text) {
  ...
  for (const part of parts) {
    if (part.startsWith('**') && part.endsWith('**')) {
      runs.push(new TextRun({ text: part.slice(2, -2), bold: true, size: 22, font: 'Lato' }));
    } else if (part) {
      runs.push(new TextRun({ text: part, size: 22, font: 'Lato' }));
    }
  }
...
app.post('/api/blogs/:blogId/publish-to-wp', async (req, res) => {
  const { data: blog, error: blogError } = await supabase
    .from('blogs')
    .select('*, clients(wp_url, wp_username, wp_app_password)')
    .eq('id', req.params.blogId)
    .single();
  ...
  // Convert markdown content to basic HTML for WP
  const htmlContent = markdownToBasicHtml(blog.content);

  const wpPayload = {
    title: blog.title_tag || blog.title,
    content: htmlContent,
    status: 'draft', // Always push as draft — someone still clicks Publish in WP
    slug: blog.slug,
    excerpt: blog.meta_description,
    meta: {
      _yoast_wpseo_metadesc: blog.meta_description,
      _yoast_wpseo_focuskw: blog.target_keyword
    }
  };
  ...
  try {
    ...
    if (!wpRes.ok) {
      ...
      return res.status(400).json({ error: `WordPress error: ${errText}` });
    }

    const wpPost = await wpRes.json();

    // Save WP post ID back to blog record
...
app.post('/api/clients/:id/test-wp', async (req, res) => {
  ...
  try {
    ...
    const testRes = await fetch(`${wpUrl}/wp-json/wp/v2/users/me`, {
      headers: { 'Authorization': `Basic ${credentials}` }
    });

    if (testRes.ok) {
      const user = await testRes.json();
      res.json({ success: true, message: `Connected as ${user.name}` });
    } else {
      res.json({ success: false, message: 'Authentication failed — check credentials' });
    }
  } catch (err) {
